...
define(
  ...
  ({ target, friction = 0.035, camx = 0, camy = 0, zoom = 200, maxzoom = 300, minzoom = 100, defaultzoom = 200 }) => {
    ...
    return template`
      ...
      <pinch-zoom ref=${pinch} target=${target} friction=${friction * 3} />
      <wheel-pan-zoom ref=${wheel} target=${target} friction=${friction * 2} />
      <touch-pan ref=${touch} target=${target} friction=${friction} />
      <tap-zoom ref=${tap} target=${target} friction=${friction * 4} />
    `
  },
)
