...
define('account-bid-card', ({ bid }) => {
  ...
  const coords = toNSEW(bid.x, bid.y)
  ...
  return html`
    ...
    <swipe-card
      class=${pending ? 'pending' : rejected ? 'rejected' : 'published'}
      onaction=${() => view()}
      onswipeleft=${() => view()}
      onswiperight=${() => goto()}
    >
      ...
      <p>
        ...
        (rejected && html`<span> <span role="status">Rejected</span> ${dateish(bid.rejection.rejected_at)} </span>`) ||
        (published && html`<span> <span role="status">Published</span> ${dateish(bid.published_at)} </span>`)},
        ${bid.amount}<i-con src="coin" dark fill></i-con> on ${coords}
      </p>
      <div slot="actions">
        <secondary-button onclick=${(e) => opts.current.controls.open({ anchor: e.target.closest('secondary-button') })}
