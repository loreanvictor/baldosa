...
use super::super::book::{ Book, BidContent, Coords };
...
pub async fn post_bid(
  ...
  {
    Ok(mut bid) => {
      if book.should_publish_immediately(&bid).await.map_err(|_| BiddingError::Unknown)? {
...
pub async fn rescind_bid(
  ...
  let tx = ledger.get_transaction(&bid.tx).await.map_err(|_| BiddingError::InvalidBid)?;
  if !tx.is_usable_offer_from(&bidder.id) {
    return Err(BiddingError::UnauthorizedTransaction);
  }

  ledger.rescind_offer(&tx, &bidder).await.map_err(|_| BiddingError::Unknown)?;
