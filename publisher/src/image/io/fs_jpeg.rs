  ...
  pub fn new(source_dir: Option<String>, target_dir: Option<String>) -> Self {
    Self {
      source_dir: source_dir.unwrap_or_else(
        || env::var("SOURCE_DIR")
          .expect("Source directory not specified for FS JPEG interface.")
      ),
      target_dir: target_dir.unwrap_or_else(
        || env::var("TARGET_DIR")
          .expect("Target directory not specified for FS JPEG interface.")
      ),
    }
  }
}
...
impl ImageInterface for FsJpegInterface {
  ...
  async fn save(
    ...
    let path = Path::new(&self.target_dir).join(target).with_extension("jpg");
  ...
  async fn delete(&self, source: &str) -> Result<String, Box<dyn Error + Send + Sync>> {
    let path = Path::new(&self.target_dir).join(source).with_extension("jpg");
